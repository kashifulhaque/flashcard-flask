...
def login():
    ...
    # Form validation logic when submit button is pressed
    if form.validate_on_submit():
        # Query the users table and fetch any rows that match the given username
...
# Logout the currenlty logged in user
@auth_blueprint.route('/logout')
def logout():
    logout_user()
    flash("You are now logged out!", "success")

    # After logging out the user, redirect them to the homepage
    return redirect(url_for('home.home'))
...
def signup():
    ...
    if form.validate_on_submit():
        # Randomly choose an avatar for the new user
        avatar = avatars[random.randint(0, 7)]
        avatar_url = 'avatars/' + avatar + '.png'

        # Put their user data in the DB
